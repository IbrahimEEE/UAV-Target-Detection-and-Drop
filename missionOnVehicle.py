# This is the whole mission code used with the Vehicle.
# Look at the mission.pdf documentation.
# There is no video recording in this code and houghCircles method is less heavy for better performance.
...
GPIO.setmode(GPIO.BOARD)    # servo setup
...
async def run():
    ...
    while first_detection_flag == 0:
        ...
        image_lab_mask_total.shape[0] / 8, param1=100, param2=40, minRadius=50, maxRadius=120)
...
async def determine_shooting_range(aldebaran22):
    ...
    gps_info_count = 0 # our uav gives 5 gps values per second
    ...
    async for velocity in aldebaran22.telemetry.velocity_ned():
        ...
        async for position in aldebaran22.telemetry.position():
            ...
            if gps_info_count == 0: # If the uav is just flying above the target (first detection),
                ...
                print("Waiting for 8 seconds... (5 gps info per sec)")
                gps_info_count += 1                   
                continue                        # we must release the load in the next tours.
                                                # so, we does not follow first 400 gps values because
            elif gps_info_count < 41:          # we are also in the shooting range now.
                ...
                if gps_info_count == 1 or gps_info_count % 5 == 0: # we print some information for
                ...
                if gps_info_count == 1 or gps_info_count % 5 == 0:
            ...
            if distance_to_target <= shooting_distance:       # when we are in the shooting range,
                ...
                if activated_servo_count == 0:
                    ...
                    await asyncio.sleep(0.3) # task wait
                    servo1.ChangeDutyCycle(0)
                    
                    print("servo activated: ",(activated_servo_count+1))
